...
const AppStore = () => {
  ...
  const sampleList = appsList.filter((app) => app.category === activeTab);
  ...
  const filteredSearchList = sampleList.filter((app) =>
        ...
        <ul className="tabItem-container">
          {tabsList.map((eachTabDetails) => (
            <TabItem
              ...
              isTabActive={eachTabDetails.tabId === activeTab}
            />
          ))}
        </ul>
        <ul className="appItem-container">
          {filteredSearchList.map((eachAppListDetails) => (
            <AppItem
              eachAppListDetails={eachAppListDetails}
              key={eachAppListDetails.appId}
            />
          ))}
        </ul>
      </div>
